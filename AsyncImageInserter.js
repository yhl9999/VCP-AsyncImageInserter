...
class AsyncImageInserter {
    constructor() {
        ...
        if (this.config.DEBUG_MODE === 'true') {
            console.log('[AsyncImageInserter] 插件初始化完成');
            console.log('[AsyncImageInserter] 配置:', this.config);
        }
    }
    ...
    async processRequest(params) {
        try {
            ...
            if (this.config.DEBUG_MODE === 'true') {
                console.log('[AsyncImageInserter] 任务已创建:', placeholderInfo.id);
                console.log('[AsyncImageInserter] 占位符:', placeholderInfo.placeholder);
            }
            ...
            return {
                ...
                taskId: placeholderInfo.id,
                message: `图片生成任务已提交，占位符：${placeholderInfo.placeholder}`,
    ...
    addTaskToQueue(task) {
        ...
        if (this.config.DEBUG_MODE === 'true') {
            console.log(`[AsyncImageInserter] 任务加入队列，位置: ${insertIndex}, 队列长度: ${this.taskQueue.length}`);
        }
    }
    ...
    async processTaskQueue() {
        ...
        if (this.config.DEBUG_MODE === 'true') {
            console.log(`[AsyncImageInserter] 开始处理任务: ${task.id}`);
        }
    ...
    async processTask(task) {
        try {
            ...
            if (this.config.DEBUG_MODE === 'true') {
                console.log(`[AsyncImageInserter] 任务完成: ${task.id}`);
                console.log(`[AsyncImageInserter] 图片URL: ${result.imageUrl}`);
            }
    ...
    async sendUpdateNotification(updateData) {
        ...
        if (this.config.DEBUG_MODE === 'true') {
            console.log('[AsyncImageInserter] 发送更新通知:', updateData);
        }

        // 临时实现：输出到控制台
        console.log('[AsyncImageInserter] PLACEHOLDER_UPDATE:', JSON.stringify(updateData));
    }
...
async function main() {
    ...
    process.stdin.on('data', async (data) => {
        console.log('[AsyncImageInserter] 收到数据:', data.toString().trim());
        ...
        console.log('[AsyncImageInserter] 分割后的行数:', lines.length);

        for (const line of lines) {
            if (line.trim()) {
                console.log('[AsyncImageInserter] 处理行:', line.trim());
                try {
                    const request = JSON.parse(line.trim());
                    console.log('[AsyncImageInserter] 解析请求:', request);
    ...
    process.stdin.on('end', async () => {
        console.log('[AsyncImageInserter] 输入流结束，剩余缓冲:', inputBuffer.trim());
        
        // 处理剩余的输入
        if (inputBuffer.trim()) {
            try {
                const request = JSON.parse(inputBuffer.trim());
                console.log('[AsyncImageInserter] 处理剩余请求:', request);
                const response = await inserter.processRequest(request);
                console.log(JSON.stringify(response));
            } catch (error) {
        ...
        if (inserter.config.DEBUG_MODE === 'true') {
            console.log('[AsyncImageInserter] 输入流处理完成');
        }
        process.exit(0);
    });
    ...
    if (inserter.config.DEBUG_MODE === 'true') {
        console.log('[AsyncImageInserter] 插件启动完成，等待输入...');
    }
    
    // 添加启动确认
    console.log('[AsyncImageInserter] 插件已启动，监听stdin输入');
}
