...
class AgentProxy {
    ...
    async sendToImageGeneratorAgent(message) {
        return new Promise((resolve, reject) => {
            ...
            // 真实调用VCP Agent系统的ImageGenerator
            // 通过HTTP API调用VCP系统
            const https = require('https');
            const http = require('http');
            const url = require('url');

            const vcpUrl = this.vcpConfig.vcpServerUrl || 'http://localhost:6005';
            const apiKey = this.vcpConfig.apiKey || 'default_key';
            ...
            const parsedUrl = url.parse(vcpUrl);
            ...
            const options = {
                ...
                path: `/api/v1/agents/${this.agentId}/chat`,
                method: 'POST',
                headers: {
                ...
                res.on('end', () => {
                    try {
                        if (res.statusCode === 200) {
                            ...
                            resolve(result.content || result.message || responseData);
                        } else {
                            console.error(`[AgentProxy] VCP API错误: ${res.statusCode}`);
                            // fallback到模拟响应
            ...
            req.on('error', (error) => {
                console.error(`[AgentProxy] HTTP请求失败:`, error);
                // fallback到模拟响应，确保插件不完全失败
                const mockResponse = this.generateMockResponse(message);
                resolve(mockResponse);
            });

            req.on('timeout', () => {
                console.error(`[AgentProxy] 请求超时`);
                req.destroy();
                // fallback到模拟响应
                const mockResponse = this.generateMockResponse(message);
                resolve(mockResponse);
